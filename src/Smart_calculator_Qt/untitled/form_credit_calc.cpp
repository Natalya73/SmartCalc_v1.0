...
void Form_Credit_Calc::CALCULATE_pressed() {
  ...
  float PervyPlatezh;     // первый платеж
  float PosledniPlatezh;  // последний платеж
  float Pereplata;        // переплата по кредиту
  float Dolg_i_Procenty;  // общая выплата
  int credit_type = DIFFERENTIAL;
  if (ui->Annuity->isChecked()) credit_type = ANNUITY;
  credit_calc(Kredit, SrokKredita, PercentKredita, credit_type, &PervyPlatezh,
