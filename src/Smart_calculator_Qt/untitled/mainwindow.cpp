...
void MainWindow::EqualButton() {
  ...
  if (validated(string_from_display) == 0) {
    ui->Result->clear();
    ui->Result->setText("Please check your input");
  } else {
    double res = string_calc(string_from_display, xxx);
...
void MainWindow::on_Graph_clicked() {
  ...
  for (double X = X_MIN_value; X <= X_MAX_value; X += step_2) {
    x.push_back(X);
    double res = 0;
    res = string_calc(string_from_display, X);
    y.push_back(res);
    i++;
  }
  ui->FieldForGraph->clearGraphs();
