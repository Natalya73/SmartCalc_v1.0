...
void form_deposit_calc::CALCULATE_clicked() {
  ...
  if (ui->ComboboxPlacements->currentText() == "monthly") {
    for (int i = 0; i < SrokDepo; i++) {
      ...
      deposit_calc(deposit, SrokDepo, PercentDepo, Nalog, capitalize,
  ...
  if (ui->Payment_frequency->currentText() == "Monthly") {
    deposit_calc_simple(deposit, SrokDepo, PercentDepo, Nalog, &PercentsTotal,
  ...
  // qDebug() << "PercentsTotal:" << PercentsTotal;
  // qDebug() << "TaxTotal:" << TaxTotal;
  // qDebug() << "DepoAndPercents:" << DepoAndPercents;
